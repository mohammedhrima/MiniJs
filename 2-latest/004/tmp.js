            ...
            props &&
                ...
                props.children.forEach((child) => {
                    ...
                    if (!child.props || !child.props.type || child.props.type === "server") HTMLvalue += generateHTML(child);
                        ...
                        // check style , children attributes
                        child.props &&
                            Object.keys(child.props)
                                .filter((key) => key != "children")
                                .forEach((key) => {
                                    attributes += ` ${key}=\"${child.props[key]}\"`;
                                });
                        HTMLvalue += `<${child.tag}${attributes}></${child.tag}>`;
                        child.props &&
                            child.props.children?.forEach((child1) => {
                                console.log("parent:", child, "child1:", child1);
                                generateDOM(`document.querySelector("[tag_id='${child.props.tag_id}']")`, child1);
                            });
                    }
                });
            HTMLvalue += `</${vdom.tag}>`;
